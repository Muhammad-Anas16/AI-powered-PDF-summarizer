...
const gemini = async (pdfText: string): Promise<GeminiResult> => {
  try {
    const chatCompletion = await groq.chat.completions.create({
      model: "llama-3.3-70b-versatile",
      messages: [
...
Respond ONLY in valid JSON like this:
{
  ...
  "answers": ["answers to any questions found in text (if any)"]
    ...
    // Always return as a single string
    try {
      const parsed = JSON.parse(aiResponse);

      // Merge everything into one formatted string
      const combinedSummary = [
        parsed.summary ? `Summary:\n${parsed.summary}` : "",
        parsed.importantPoints?.length
          ? `\n\nImportant Points:\n- ${parsed.importantPoints.join("\n- ")}`
          : "",
      ...
      return { summary: combinedSummary || aiResponse };
    } catch {
      console.warn("⚠️ AI did not return valid JSON, raw response:", aiResponse);
